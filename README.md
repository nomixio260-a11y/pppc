...
## 最短で試す（サーバー不要・ダウンロードして開くだけ）

**`public/anp.html`** は全部入りの単一HTMLファイル。ダウンロードしてブラウザで開くだけで動く（インストールも起動もサーバーも不要）。

1. GitHub でリポジトリの **`public/anp.html`** を開き「Download raw file」で保存（または Actions の実行結果 → Artifacts → `anp-single-file-html` から取得）
2. 保存した `anp.html` をブラウザで開く（ダブルクリック）
3. 2人でつなぐ:
   - **相手A（招待する人）**: 「ネットワークを作成して参加」→「オフラインで招待」で相手の参加コードからオファーを作る
   - **相手B（参加する人）**: 「オフラインで参加」→「参加コードを作成」してAに渡す → Aのオファーを貼ってアンサーを作りAへ返す
   - テキストを1往復コピペするだけで、ブラウザ同士が直接つながりチャット・ファイル共有できる

> オフライン接続は Relay もサーバーも一切使わない（WebRTCのSDPを人手で交換する方式）。`file://` で開いても Web Crypto は使えるので動作する。ただし永続化(IndexedDB)が使えない環境では、リロードで鍵・履歴が消える（「identity をエクスポート」で保存可）。多人数メッシュや自動発見を使いたい場合は下の Relay 版を使う。

## クイックスタート（Relay版・自動発見つき）
...
| 13 | オフライン接続（コピペ signaling）+ 単一HTML + `file://` メモリフォールバック | Relayもサーバーも不要、ファイルを開くだけで2者接続 |
